...
export async function POST({ request, cookies }) {
  ...
  if (createErr) {
    ...
    if (!alreadyExists) {
      console.error('[api/create-admin-user] createUser:', createErr.message);
    ...
    if (updErr || !updated?.user) {
      console.error('[api/create-admin-user] updateUser:', updErr?.message);
      return json(
    ...
    if (reinErr) {
      console.warn('[api/create-admin-user] reinforce metadata:', reinErr.message);
  ...
  if (linkErr) {
    console.error('[api/create-admin-user] link user_id:', linkErr.message);
    return json(
      {
...
async function findUserByEmail(service, email) {
  ...
  for (let page = 1; page <= 10; page += 1) {
    ...
    if (error) {
      console.error('[api/create-admin-user] listUsers:', error.message);
      return null;
    }
