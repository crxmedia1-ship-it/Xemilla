...
/**
 * SuperAdmin: prende/apaga solo gadget_ar sin tocar el resto de Identidad.
...
export async function POST({ request, cookies }) {
  ...
  const { data, error } = await writeClient
    .from('restaurantes')
    .update({ gadget_ar: Boolean(activo) })
    .eq('id', restauranteId)
    .select('id, nombre_comercial, slug, gadget_ar')
    .maybeSingle();

  if (error) {
    if (/gadget_ar|column|does not exist|schema cache/i.test(error.message || '')) {
      return json(
        {
          error:
            'Columna gadget_ar no existe. Ejecutá la migración AR en Supabase.',
        },
        400,
      );
    }
    console.error('[api/update-gadget-ar]', error.message);
  ...
  return json({
    ok: true,
    restaurante: data,
    message: data.gadget_ar
      ? 'Gadget AR activo. En Menú y WebApp aparece Ver en AR cuando el plato tiene modelo 3D.'
      : 'Gadget AR apagado. Sin rastro en admin ni WebApp.',
  });
}
